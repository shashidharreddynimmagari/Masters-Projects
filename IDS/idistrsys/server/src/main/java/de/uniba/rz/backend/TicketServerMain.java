...
public class TicketServerMain {
	...
	private static List<RemoteAccess> getAvailableRemoteAccessImplementations(String[] args) {
		...
		switch (args[0].toLowerCase()) {
			case "udp":
				implementations.add(new UdpRemoteAccess(args[1]));
				break;
			case "amqp":
				implementations.add(new AMQPRemoteAccess(args[1]));
				break;
			case "rest":
				implementations.add(new RESTfulRemoteAccess(args[1]));
				break;
			case "grpc":
				implementations.add(new gRPCRemoteAccess(args[1]));
			default:
				implementations.add(new UdpRemoteAccess(args[1]));
				break;
		}
